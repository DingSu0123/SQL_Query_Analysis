...
# -*- coding: utf-8 -*-
"""
Created on Thu Nov 24 15:02:00 2022
...
if __name__ == '__main__':
    ...
    sql = """ 
           select e.MANAGER_SUB_BRANCH_CODE,a.BRANCH_NAME,count(distinct a.PROD_NAME) as CNT,
  	sum(convert(number,a.ASSET_TG)) as ASSET_TG,
  	    sum(isnull(convert(float,a.ASSET_TG_FEE_YS),0))/10000 as ASSET_TG_FEE_YS,
    sum(isnull(convert(float,a.ASSET_TG_FEE_SS),0))/10000 as ASSET_TG_FEE_SS,
    sum(isnull(convert(float,a.ASSET_WB_FEE_YS),0))/10000 as ASSET_WB_FEE_YS,
    sum(isnull(convert(float,a.ASSET_WB_FEE_SS),0))/10000 as ASSET_WB_FEE_SS
    from ST3200.PB a
    right join SC3200.VW_ORG_EMPLOYEE e
    on a.BRANCH_NAME=e.DEPT_NAME
    group by e.MANAGER_SUB_BRANCH_CODE,a.BRANCH_NAME limit  100
